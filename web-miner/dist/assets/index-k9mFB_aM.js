(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))a(n);new MutationObserver(n=>{for(const i of n)if(i.type==="childList")for(const l of i.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&a(l)}).observe(document,{childList:!0,subtree:!0});function t(n){const i={};return n.integrity&&(i.integrity=n.integrity),n.referrerPolicy&&(i.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?i.credentials="include":n.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function a(n){if(n.ep)return;n.ep=!0;const i=t(n);fetch(n.href,i)}})();class B{constructor(e){this.poolUrl=e}ws=null;state="disconnected";handlers={};reconnectAttempts=0;maxReconnectAttempts=5;reconnectDelay=1e3;minerId=null;pendingAddress=null;setHandlers(e){this.handlers=e}getState(){return this.state}getMinerId(){return this.minerId}async connect(e){if(this.state!=="disconnected")throw new Error(`Cannot connect: already ${this.state}`);return this.pendingAddress=e,this.setState("connecting"),new Promise((t,a)=>{try{this.ws=new WebSocket(this.poolUrl),this.ws.onopen=()=>{this.reconnectAttempts=0,this.setState("registering"),this.send({type:"register",address:e}),t()},this.ws.onclose=n=>{console.log(`WebSocket closed: code=${n.code}, reason=${n.reason}`),this.handleDisconnect()},this.ws.onerror=n=>{console.error("WebSocket error:",n),this.state==="connecting"&&a(new Error("Failed to connect to pool"))},this.ws.onmessage=n=>{this.handleMessage(n.data)}}catch(n){this.setState("disconnected"),a(n)}})}disconnect(){this.ws&&(this.ws.close(1e3,"Client disconnect"),this.ws=null),this.minerId=null,this.setState("disconnected")}sendReady(){if(this.state!=="ready"&&this.state!=="mining"&&this.minerId===null)throw new Error("Cannot send ready: not registered");this.send({type:"ready"})}submitResult(e,t,a,n,i){this.send({type:"job_result",job_id:e,nonce:t,work:a,hash_count:n,elapsed_time:i})}setState(e){this.state!==e&&(this.state=e,this.handlers.onStateChange?.(e))}send(e){if(!this.ws||this.ws.readyState!==WebSocket.OPEN){console.error("Cannot send: WebSocket not connected");return}this.ws.send(JSON.stringify(e))}handleMessage(e){let t;try{t=JSON.parse(e)}catch(a){console.error("Failed to parse pool message:",a);return}switch(t.type){case"registered":this.minerId=t.miner_id,this.setState("ready"),this.handlers.onRegistered?.(t.miner_id),this.sendReady();break;case"new_job":this.setState("mining"),this.handlers.onNewJob?.(t);break;case"block_won":this.handlers.onBlockWon?.(t.block_number,t.job_id);break;case"job_completed":this.handlers.onJobCompleted?.(t.job_id,t.you_won),this.setState("ready");break;case"error":console.error("Pool error:",t.message),this.handlers.onError?.(t.message);break;default:console.warn("Unknown message type:",t)}}handleDisconnect(){this.ws=null;const e=this.state!=="disconnected"&&this.state!=="connecting";if(this.setState("disconnected"),e&&this.pendingAddress&&this.reconnectAttempts<this.maxReconnectAttempts){this.reconnectAttempts++;const t=this.reconnectDelay*Math.pow(2,this.reconnectAttempts-1);console.log(`Reconnecting in ${t}ms (attempt ${this.reconnectAttempts}/${this.maxReconnectAttempts})`),setTimeout(()=>{this.pendingAddress&&this.connect(this.pendingAddress).catch(a=>{console.error("Reconnection failed:",a)})},t)}}}const L=`// Quantus Mining Shader - WGSL Compute Shader for GPU Mining
...
`,p=33,N=8,T=16,U=16,A=4,O=1,M=256;class y{device=null;pipeline=null;bindGroupLayout=null;resultsBuffer=null;headerBuffer=null;nonceBuffer=null;difficultyBuffer=null;dispatchConfigBuffer=null;cancelFlagBuffer=null;resultsStagingBuffer=null;currentJob=null;isMining=!1;shouldCancel=!1;workgroupsPerDispatch=256;noncesPerThread=16;static isSupported(){return"gpu"in navigator}async initialize(){if(!y.isSupported())throw new Error("WebGPU is not supported in this browser");const e=await navigator.gpu.requestAdapter({powerPreference:"high-performance"});if(!e)throw new Error("Failed to get GPU adapter");this.device=await e.requestDevice({requiredLimits:{maxStorageBufferBindingSize:e.limits.maxStorageBufferBindingSize,maxComputeWorkgroupsPerDimension:e.limits.maxComputeWorkgroupsPerDimension}}),this.device.lost.then(n=>{console.error("GPU device lost:",n.message),this.cleanup()});const t=this.device.createShaderModule({code:L}),a=await t.getCompilationInfo();for(const n of a.messages){if(n.type==="error")throw new Error(`Shader compilation error: ${n.message}`);console.warn(`Shader warning: ${n.message}`)}this.bindGroupLayout=this.device.createBindGroupLayout({entries:[{binding:0,visibility:GPUShaderStage.COMPUTE,buffer:{type:"storage"}},{binding:1,visibility:GPUShaderStage.COMPUTE,buffer:{type:"read-only-storage"}},{binding:2,visibility:GPUShaderStage.COMPUTE,buffer:{type:"read-only-storage"}},{binding:3,visibility:GPUShaderStage.COMPUTE,buffer:{type:"read-only-storage"}},{binding:4,visibility:GPUShaderStage.COMPUTE,buffer:{type:"read-only-storage"}},{binding:5,visibility:GPUShaderStage.COMPUTE,buffer:{type:"read-only-storage"}}]}),this.pipeline=this.device.createComputePipeline({layout:this.device.createPipelineLayout({bindGroupLayouts:[this.bindGroupLayout]}),compute:{module:t,entryPoint:"mining_main"}}),this.createBuffers(),console.log("GPU miner initialized successfully")}createBuffers(){if(!this.device)throw new Error("Device not initialized");this.resultsBuffer=this.device.createBuffer({size:p*4,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_SRC|GPUBufferUsage.COPY_DST}),this.headerBuffer=this.device.createBuffer({size:N*4,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_DST}),this.nonceBuffer=this.device.createBuffer({size:T*4,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_DST}),this.difficultyBuffer=this.device.createBuffer({size:U*4,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_DST}),this.dispatchConfigBuffer=this.device.createBuffer({size:A*4,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_DST}),this.cancelFlagBuffer=this.device.createBuffer({size:O*4,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_DST}),this.resultsStagingBuffer=this.device.createBuffer({size:p*4,usage:GPUBufferUsage.MAP_READ|GPUBufferUsage.COPY_DST})}async mine(e){if(!this.device||!this.pipeline||!this.bindGroupLayout)throw new Error("GPU not initialized");if(this.isMining)throw new Error("Already mining");this.currentJob=e,this.isMining=!0,this.shouldCancel=!1;try{const t=new Uint32Array(p);this.device.queue.writeBuffer(this.resultsBuffer,0,t.buffer),this.device.queue.writeBuffer(this.headerBuffer,0,e.header.buffer),this.device.queue.writeBuffer(this.difficultyBuffer,0,e.difficultyTarget.buffer),this.device.queue.writeBuffer(this.nonceBuffer,0,e.startNonce.buffer),this.device.queue.writeBuffer(this.cancelFlagBuffer,0,new Uint32Array([0]).buffer);let a=0;const n=new Uint32Array(e.startNonce);for(;!this.shouldCancel;){const i=await this.dispatchMining(n);if(a+=i.hashCount,e.hashCount=a,i.found&&i.nonce&&i.hash)return{found:!0,nonce:i.nonce,hash:i.hash,hashCount:a};this.advanceNonce(n,i.hashCount),await new Promise(l=>setTimeout(l,0))}return{found:!1,hashCount:a}}finally{this.isMining=!1,this.currentJob=null}}cancel(){this.shouldCancel=!0,this.device&&this.cancelFlagBuffer&&this.device.queue.writeBuffer(this.cancelFlagBuffer,0,new Uint32Array([1]).buffer)}isBusy(){return this.isMining}getCurrentJob(){return this.currentJob}cleanup(){this.cancel(),this.resultsBuffer?.destroy(),this.headerBuffer?.destroy(),this.nonceBuffer?.destroy(),this.difficultyBuffer?.destroy(),this.dispatchConfigBuffer?.destroy(),this.cancelFlagBuffer?.destroy(),this.resultsStagingBuffer?.destroy(),this.device?.destroy(),this.device=null,this.pipeline=null,this.bindGroupLayout=null}async dispatchMining(e){if(!this.device||!this.pipeline||!this.bindGroupLayout)throw new Error("GPU not initialized");const t=this.workgroupsPerDispatch*M,a=t*this.noncesPerThread;this.device.queue.writeBuffer(this.nonceBuffer,0,e.buffer);const n=new Uint32Array([t,this.noncesPerThread,a,64]);this.device.queue.writeBuffer(this.dispatchConfigBuffer,0,n.buffer);const i=this.device.createBindGroup({layout:this.bindGroupLayout,entries:[{binding:0,resource:{buffer:this.resultsBuffer}},{binding:1,resource:{buffer:this.headerBuffer}},{binding:2,resource:{buffer:this.nonceBuffer}},{binding:3,resource:{buffer:this.difficultyBuffer}},{binding:4,resource:{buffer:this.dispatchConfigBuffer}},{binding:5,resource:{buffer:this.cancelFlagBuffer}}]}),l=this.device.createCommandEncoder(),h=l.beginComputePass();h.setPipeline(this.pipeline),h.setBindGroup(0,i),h.dispatchWorkgroups(this.workgroupsPerDispatch),h.end(),l.copyBufferToBuffer(this.resultsBuffer,0,this.resultsStagingBuffer,0,p*4),this.device.queue.submit([l.finish()]),await this.resultsStagingBuffer.mapAsync(GPUMapMode.READ);const b=new Uint32Array(this.resultsStagingBuffer.getMappedRange().slice(0));if(this.resultsStagingBuffer.unmap(),b[0]!==0){const I=new Uint32Array(b.slice(1,17)),E=new Uint32Array(b.slice(17,33));return{found:!0,nonce:I,hash:E,hashCount:a}}return{found:!1,hashCount:a}}advanceNonce(e,t){let a=t;for(let n=0;n<e.length&&a>0;n++){const i=e[n]+a;e[n]=i>>>0,a=i>4294967295?1:0}}}function R(r,e){const t=e*8;r=r.padStart(t,"0");const a=new Uint32Array(e);for(let n=0;n<e;n++){const i=t-(n+1)*8,l=r.slice(i,i+8);a[n]=parseInt(l,16)}return a}function D(r){let e="";for(let t=r.length-1;t>=0;t--)e+=r[t].toString(16).padStart(8,"0");return e.replace(/^0+/,"")||"0"}function z(r){const e=new Uint32Array(16);if(r==="0"||r==="")return e;let t=BigInt(r);const a=BigInt(4294967295);for(let n=0;n<16&&t>0n;n++)e[n]=Number(t&a),t>>=32n;return e}function H(r,e){let t="";for(let a=0;a<r.length;a++)t+=r[a].toString(16).padStart(8,"0");for(let a=7;a>=0;a--){const n=e[a],i=n>>>24&255,l=n>>>16&255,h=n>>>8&255,b=n&255;t+=b.toString(16).padStart(2,"0"),t+=h.toString(16).padStart(2,"0"),t+=l.toString(16).padStart(2,"0"),t+=i.toString(16).padStart(2,"0")}return t}let c=null,d=null,f=null,o={hashRate:0,totalHashes:0,blocksWon:0,jobsCompleted:0,uptime:0},x=null,g=null,v=0,S=0;function s(r){return document.getElementById(r)}function _(r){const e=s("status");e&&(e.textContent=r)}function w(r){const e=s("state");e&&(e.textContent=r,e.className=`state-${r}`);const t=s("connect-btn"),a=s("disconnect-btn"),n=s("address");t&&(t.disabled=r!=="disconnected"),a&&(a.disabled=r==="disconnected"),n&&(n.disabled=r!=="disconnected")}function m(){const r=s("hash-rate"),e=s("total-hashes"),t=s("blocks-won"),a=s("jobs-completed"),n=s("uptime");r&&(r.textContent=W(o.hashRate)),e&&(e.textContent=P(o.totalHashes)),t&&(t.textContent=o.blocksWon.toString()),a&&(a.textContent=o.jobsCompleted.toString()),n&&(n.textContent=$(o.uptime))}function F(r){const e=s("job-id"),t=s("job-hashes");e&&(e.textContent=r?.jobId??"-"),t&&(t.textContent=r?P(r.hashCount):"-")}function u(r,e="info"){const t=s("log");if(!t)return;const a=document.createElement("div");for(a.className=`log-entry log-${e}`,a.textContent=`[${new Date().toLocaleTimeString()}] ${r}`,t.appendChild(a),t.scrollTop=t.scrollHeight;t.children.length>100;)t.removeChild(t.firstChild)}function W(r){return r>=1e9?`${(r/1e9).toFixed(2)} GH/s`:r>=1e6?`${(r/1e6).toFixed(2)} MH/s`:r>=1e3?`${(r/1e3).toFixed(2)} KH/s`:`${r.toFixed(2)} H/s`}function P(r){return r>=1e12?`${(r/1e12).toFixed(2)}T`:r>=1e9?`${(r/1e9).toFixed(2)}B`:r>=1e6?`${(r/1e6).toFixed(2)}M`:r>=1e3?`${(r/1e3).toFixed(2)}K`:r.toString()}function $(r){const e=Math.floor(r/3600),t=Math.floor(r%3600/60),a=Math.floor(r%60);return`${e.toString().padStart(2,"0")}:${t.toString().padStart(2,"0")}:${a.toString().padStart(2,"0")}`}async function q(r){if(!d){u("GPU miner not initialized","error");return}d.isBusy()&&(d.cancel(),await new Promise(n=>setTimeout(n,100)));const e=R(r.mining_hash,8),t=z(r.difficulty),a=new Uint32Array(16);crypto.getRandomValues(a),f={jobId:r.job_id,header:e,difficultyTarget:t,startNonce:a,hashCount:0,startTime:Date.now()},u(`Starting job ${r.job_id}`,"info"),F(f),j(f).catch(n=>{u(`Mining error: ${n.message}`,"error")})}async function j(r){if(!d||!c)return;const e=await d.mine(r);if(o.totalHashes+=e.hashCount,e.found&&e.nonce&&e.hash){u(`Solution found for job ${r.jobId}!`,"success");const t=D(e.nonce),a=H(r.header,e.nonce),n=(Date.now()-r.startTime)/1e3;c.submitResult(r.jobId,t,a,e.hashCount,n),u(`Submitted solution: nonce=${t.slice(0,16)}...`,"info")}F(null),f=null}function C(){d?.isBusy()&&(d.cancel(),u("Job cancelled","warning")),f=null,F(null)}function J(){v=o.totalHashes,S=Date.now(),g=window.setInterval(()=>{const r=Date.now(),e=(r-S)/1e3;if(e>0){const t=o.totalHashes-v;o.hashRate=t/e,f&&d?.isBusy()&&(f.hashCount,F(f))}v=o.totalHashes,S=r,x&&(o.uptime=(Date.now()-x)/1e3),m()},1e3)}function Y(){g!==null&&(clearInterval(g),g=null)}async function k(){const r=s("address"),e=s("pool-url"),t=r?.value.trim(),a=e?.value.trim()||"wss://localhost:9834";if(!t){u("Please enter a rewards address","error");return}if(t.length<40||t.length>50){u("Invalid address format (expected SS58)","error");return}_("Initializing GPU..."),u("Initializing WebGPU...","info");try{if(!d){if(!y.isSupported())throw new Error("WebGPU is not supported in this browser. Try Chrome/Edge 113+ or enable WebGPU in flags.");d=new y,await d.initialize(),u("WebGPU initialized successfully","success")}_("Connecting to pool..."),u(`Connecting to ${a}...`,"info"),c=new B(a),c.setHandlers({onStateChange:n=>{w(n),u(`State: ${n}`,"info")},onRegistered:n=>{u(`Registered with miner ID: ${n}`,"success"),x=Date.now(),J()},onNewJob:n=>{u(`New job: ${n.job_id}`,"info"),q(n)},onBlockWon:(n,i)=>{o.blocksWon++,u(`Block ${n} won! (job: ${i})`,"success"),m()},onJobCompleted:(n,i)=>{o.jobsCompleted++,i||(C(),u(`Job ${n} completed by another miner`,"warning")),m()},onError:n=>{u(`Pool error: ${n}`,"error")}}),await c.connect(t),_("Connected and mining")}catch(n){const i=n instanceof Error?n.message:String(n);u(`Connection failed: ${i}`,"error"),_("Connection failed"),w("disconnected")}}function X(){C(),Y(),c&&(c.disconnect(),c=null),_("Disconnected"),w("disconnected"),u("Disconnected from pool","info"),o={hashRate:0,totalHashes:0,blocksWon:0,jobsCompleted:0,uptime:0},x=null,m()}function G(){if(!y.isSupported()){_("WebGPU not supported"),u("WebGPU is not supported in this browser","error"),u("Try Chrome/Edge 113+ or enable WebGPU in browser flags","info");const a=s("connect-btn");a&&(a.disabled=!0);return}u("WebGPU is supported","success"),_("Ready to connect"),w("disconnected"),m();const r=s("connect-btn"),e=s("disconnect-btn");r&&r.addEventListener("click",()=>{k().catch(a=>{console.error("Connect error:",a)})}),e&&e.addEventListener("click",X);const t=s("address");t&&t.addEventListener("keypress",a=>{a.key==="Enter"&&k().catch(n=>{console.error("Connect error:",n)})})}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",G):G();
